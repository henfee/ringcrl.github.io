<template>
  <div id="app">
    <mu-appbar :zDepth="0" :title="title === 'index' ? '' : cTitel" class="example-appbar" :class="{'nav-hide': !open}">
      <mu-icon-button @click="toggleNav" icon="menu" slot="left" />
      <mu-icon-button slot="right" href="https://github.com/ringcrl" target="_blank" icon=":mudocs-icon-custom-github" />
    </mu-appbar>
...
export default {
  data() {
    ...
    computed: {
      cTitel: function() {
        if (this.title === 'appbar') this.title = '组件-AppBar'
        if (this.title === 'contributing') this.title = '贡献指南'
        return this.title
      }
    },
    destroyed() {
